...
// ==========================================================================
// READ MORE — mobile-only text truncation, shared across pages
// Wrap prose like:
// <div class="text-clamp">...paragraphs...</div>
// <button class="read-more-btn" data-clamp-toggle><span class="label">Read more</span><span class="chev">▾</span></button>
// The button only appears if the text actually overflows at the current
// width, and disappears again above the mobile breakpoint.
// ==========================================================================
function initReadMore(clampEl) {
  const btn = clampEl.nextElementSibling;
  if (!btn || !btn.matches('[data-clamp-toggle]')) return;

  const label = btn.querySelector('.label');
  let expanded = false;

  function checkOverflow() {
    if (expanded) return; // don't hide the button once opened, until re-collapsed
    const overflowing = clampEl.scrollHeight > clampEl.clientHeight + 2;
    btn.classList.toggle('has-overflow', overflowing);
  }

  btn.addEventListener('click', () => {
    expanded = !expanded;
    clampEl.classList.toggle('expanded', expanded);
...
document.querySelectorAll('.text-clamp').forEach(initReadMore);
